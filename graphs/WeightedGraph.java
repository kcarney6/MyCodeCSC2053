...
public class WeightedGraph<T> implements WeightedGraphInterface<T>
{
  ...
  {
	  vertices[numVertices] = vertex;//for the first time, at array 0, put the vertex
	  for(int i =0;i<numVertices;i++) {
		  edges[numVertices][i] = NULL_EDGE;//it is null because we have not done the add edge method yet
  ...
  {
	  //bellow is the code to search the vertecies array
	  for(int i=0;i<numVertices;i++) {
		  if(vertices[i].equals(vertex)) {
			  return i; //returns the index place when it equals the one for our vertex
		  }
	  }
	  return -1;
  }
  ...
  // Adds an edge with the specified weight from fromVertex to toVertex.
  {
   ...
   edges [row][col] = weight;//makes a row and a collumn
